...
## OPEN
- [2026-05-14] **TT iter#3 — singular `'аккаунт'` trigger** — smoke task 5796 подтвердил: settings-хоп iter#2 работает, приземляется на страницу настроек, но строка там — `«Аккаунт»` (singular), а `TT_DRAWER_ACCOUNT_TRIGGERS` имеет только `аккаунты`/`accounts`. Фикс: добавить `'аккаунт'` в список → тесты → PR → деплой → smoke. Возможный iter#4 если tap «Аккаунт» откроет ещё страницу. Затем 24h soak. Evidence: `docs/evidence/2026-05-14-tt-account-switcher-settings-nested-shipped.md` § Live verify. | от: self (TT триаж 2026-05-14)
