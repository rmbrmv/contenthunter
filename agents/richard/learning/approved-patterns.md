...
[2026-04-01] Технический ревью с Генри через sessions_send | Быстро поделили задачи, каждый закрыл свою часть | Паттерн парной работы: 1) один изучает код и пишет первый анализ, 2) второй отвечает с дополнениями и уточнениями, 3) делят по специализации. Не тратить время на то что другой знает лучше.

[2026-04-01] Проверка 401 на живом endpoint | `curl -X POST /api/upload/generate-description` с реальным токеном → 200 | Перед заменой ключа/токена — всегда проверить endpoint вручную. Если 200 — проблема не в ключе а в клиенте.

[2026-04-01] `savingPreference` + `lastSavedStatus` для UX feedback | Toast исчезает через 1.5с через setTimeout | Паттерн: ref для состояния + setTimeout для auto-clear. Не нужна библиотека для простых toast.

[2026-04-01] Контекст агента из БД в system prompt | `SELECT project, onboarding_stage FROM validator_projects` → добавить в system prompt | Агент без контекста = слепой. Всегда передавать минимум: имя проекта + текущий этап.
