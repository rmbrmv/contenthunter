...
> ⚠️ **Поправки по факту исполнения 27.05** (детали в `docs/evidence/2026-05-27-wp122-tt-share-loop-activation.md`):
> - **Task 3:** НЕ запускать `publisher.py <id>` вручную. Testbench-публикатор (PM2 id 33 `autowarm-testbench`) САМ диспатчит pending `testbench=true` задачи → канонический смок = только INSERT pending-строки, диспетчер запустит. Ручной запуск создал гонку двух процессов на устройстве.
> - **Task 4:** Рестарт `pm2 35` НЕ нужен (и вреден — `pkill -f publisher.py` на старте прервёт in-flight, а прод публикует непрерывно). `publisher.py` делает `load_dotenv` на каждый спавн → достаточно правки прод `.env`, новые публикации подхватят флаг, in-flight не тронуты.
