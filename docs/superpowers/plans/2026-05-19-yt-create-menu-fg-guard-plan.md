...
Запустить каждые 60 секунд (или вручную):

```bash
while true; do
  PGPASSWORD=openclaw123 psql -h localhost -U openclaw -d openclaw <<'SQL'
SELECT pt.id, pt.account, pt.status, pt.error_code, pt.post_url
FROM publish_tasks pt
...
Heredoc (`<<'SQL'`) исключает двойную интерпретацию quoting'а (single-quoted SQL литералы `'YouTube'` / `'30 minutes'` дойдут до psql как есть; иначе оболочка/`watch` ломает экранирование).
