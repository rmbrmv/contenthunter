...
**Approach A** (метрический учёт, без мутации `publish_queue`). Решения Данила: учёт = **авто-успех** (не ручная, не «Потеряно»); «Отменить» → **«В работе»** (`in_progress`); ссылка **опциональна** и пишется только в строку очереди (не в `slot.matched_post_url` — чистые метрики).

Сделано: новый терминальный `operator_status='published_auto'` (миграция CHECK + rollback `20260529_wp187_published_auto_status`); переходы `markPublishedAuto`/`cancelPublishedAuto` (`manual_publish_queue.js`); роуты `publish-auto`/`cancel-auto` + kill-switch `MANUAL_PUBLISHED_AUTO_ENABLED` (`server.js`); фронт — кнопки «Выложено авто»/«Отменить» + фиолетовый бейдж, `mpqAgg`→`mpq_pure.js` с учётом нового статуса (пак = «Выложено», когда все строки `published`/`published_auto`); воронка (`pipeline_funnel.js`) — `auto_acknowledged` в «Авто» с дедупом против `publish_queue.done`, Q4-исключение, `manual_inflight` минус handoff-ack (матч по `unic_result_id+account+platform`); строка в дашборде и TG-отчёте (`daily_publish_report.js`).
...
### WP #137 апдейт (2026-06-03 пост-шип): что закрыто / что осталось

**Закрыто в тот же день:**
- ✅ Laozhang-ключ: старый был невалиден (не баланс — отозван); заменён на рабочий, смок gpt-image-1 edit OK.
- ✅ Баг источника лого (читалось из пустой `validator_projects.logo_url` вместо `validator_brand_profiles`) — в роутере И в фоновом воркере; единый `resolve_project_logo_url` (main 88c7ca0).
- ✅ Параллелизация генерации: concurrency=3 + timeout=180с (при 5 Laozhang таймаутит — server-side queue, не rate-limit); 5/5 за ~128с (main d31ef7f). Live E2E пройден.

**Осталось (не блокеры):**
- Гейт `LOGO_VARIANT_GATE_ENABLED` оставлен OFF по решению Данила — генерация в проде работает, фича клиентам не видна. Включение = флаг + `pm2 restart 24` (затрагивает всех клиентов, US-2).
- Карточка на выходе ~765px вместо 900px (gpt-image отдаёт «тяжёлый» PNG, weight-loop ужимает до ≤500КБ). Поднять лимит веса или палитру.
- Текст ожидания «1–2 минуты» занижен для наборов из 10 (~3.5–4 мин).
- Онбординг-тур для нового шага (ТЗ «если надо»).
- `reset` не чистит объекты S3 (подберёт TTL-крон).
- `_call_image_model`: docstring обещает фолбэк edits→generations, код не реализует — реализовать или убрать обещание.

## WP#152 element-positions follow-up (2026-06-03 пост-шип)

Позиции элементов схем 35-70 (лицо+лого только в верхних углах) задеплоены, OP#152→Тестирование. Остаток (не блокеры):
- **Verify-приёмка:** перегенерировать превью при заведении проекта клиента → визуально подтвердить лицо+лого в верхних углах без пересечений; verify после 1-й реальной выкладки (старт 10 июня).
- (минор) `worker.py`: паттерн `.get(k,default)`-vs-None всплыл уже 3-й раз в unic-пайплайне (NULL logo_*/pattern_*/position). Кандидат — обёртка нормализации схемы при чтении из БД (None→дефолт по словарю колонок), чтобы не латать точечно. Отдельным тех-долгом.
- (минор) `or 0`/`or 10` для offset коллапсит легитимный 0 в дефолт; для 35-70 не важно (offset=20), но для общего пути — поведенческое изменение. CONCERN-2 ревью, не блокер.

## WP#225 follow-up (2026-06-04 пост-шип): root-cause схлопнутого профиля

Фикс WP#225 (escape рилс-дрейфа, kill-switch `IG_PICKER_REEL_DRIFT_ESCAPE_ENABLED`) делает шаг открытия sheet выбора аккаунта устойчивым к состоянию «профиль показывает сетку Reels от верха / открыт рилс-пост», но **не устраняет первопричину** появления этого состояния — почему навигация до switcher'а иногда оставляет профиль схлопнутым / в Reels-таб (хедер с username-шевроном не отрисован). Follow-up (если после деплоя событие `ig_picker_reguard_reel_drift_escaped` окажется частым): разобрать навигацию `_go_to_profile_tab` → открытие switcher'а (возможно, тап по profile-табу попадает на уже-открытый Reels-таб или скролл не доезжает до верха). Не блокер — escape+suppression уже не дают ложного фейла.

## 2026-06-04 — WP#247 Фаза 2 follow-up (пост-шип миграции naive→timestamptz)

Фаза 2 (миграция 7 колонок publish_queue/publish_tasks → timestamptz + флип читателей) задеплоена, OP#247→Тестирование. Память: `project_wp247_per_user_display_timezone`. Не блокеры:

- **(корректность/UX) `index.html:8196 submitPublishTask` шлёт сырую `datetime-local`-строку без оффсета.** Два соседних пути (12199, autowarm 4994) конвертят в UTC-ISO (`new Date(...).toISOString()`/`tzLocalToUTC`), а ручная публикация через 8196 — нет. Сейчас не риск (система трактует scheduled_at как UTC и до, и после миграции — момент сохранён), но оператор, вводящий локальное время, получает скрытый сдвиг. Привести 8196 к тому же UTC-преобразованию, что и соседи. → **OP follow-up создан.**
- **(тех-долг) `approval_notify.js` собственный `MSK_OFFSET_MS`.** Отдельный notify-путь без пользователя, МСК-корректен, вне scope Фазы 1/2 — кандидат свести на `tz_display`.
- **(косметика) `publish_planner.js:164/327` переменная `NAIVE` теперь держит `tzClause(TZ)`** (имя вводит в заблуждение) и стала дублем `TS` в getPlannerCards. Имя сохранено ради минимального дифа при флипе; переименовать `NAIVE→TS`/схлопнуть при следующем касании файла.
- **(тех-долг процесса) аудит читателей при tz-миграциях должен искать и РУКОПИСНЫЕ `AT TIME ZONE 'UTC' AT TIME ZONE`, не только хелпер `naiveTzClause`.** Именно так финальный холистический ревью поймал CRITICAL в `retry_controller.js` (grep по хелперу его не видел).
