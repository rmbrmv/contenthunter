...
## WP#239 follow-up (2026-06-04 пост-шип)

Единый переключатель типа даты дашборда (`date_basis=planned|task`, дефолт planned) задеплоен в прод (delivery main 6745b53, PM2 id35 restart), OP#239→Тестирование. Остаток (не блокеры, выявлены на финальном ревью):
- **Verify-приёмка (UI):** Данилу — открыть «📊 Дашборд выкладки», переключить «Запланировано ↔ Дата задачи», убедиться что плитки/тренд/воронка пересчитываются; проверить телеграм-отчёт (два раздела). Старт по следующему ежедневному отчёту (09:50 МСК).
- **(минор) `buildErrorBreakdown` не basis-aware:** причины ошибок в телеграм-отчёте считаются по `scheduled_at`-окну для ОБОИХ разделов (в т.ч. для «по запланированной дате»). Осознанное упрощение (комментарий в коде) — для дневного отчёта scheduled_at и slot_date обычно в одном дне. Кандидат на доведение, если разделы будут заметно расходиться.
- **(минор) UI: базис не сохраняется в URL-закладке.** `switchDashboardBasis` не зовёт `setSubParam` (в отличие от пресетов) → рефреш страницы сбрасывает на planned. YAGNI вне скоупа WP#239; поднять, если закладки на «дату задачи» войдут в рабочий процесс.
