...
def _is_tt_feed_after_pick(self, xml: str, header_y_max: int = 260) -> bool:
    ...
    Returns True if ≥2 *distinct* markers из `_TT_FEED_MARKERS` присутствуют
    в top header_y_max px. Threshold 2/3 conservative: single marker
    insufficient (could be unrelated label), 2+ distinct — strong signal.

    Distinct-tracking (set), а не counter: TT часто дублирует label в
    `text` и `content-desc` смежных elements (e.g. "Смотреть" в обеих
    нодах) — counter дал бы false-positive на одном marker'е. Set
    гарантирует threshold = distinct labels.
    """
    if not xml:
        return False
...
2. **test_is_tt_feed_one_marker_returns_false** — 1/3 distinct → `False` (threshold ≥2).
3. **test_is_tt_feed_duplicate_marker_returns_false** — same marker "Смотреть" появляется в 2 different elements (text vs content-desc, типичный TT layout) → distinct set {Смотреть} → `False`. (Защита от counter-based false-positive — поймано Codex round 1.)
4. **test_is_tt_feed_below_header_returns_false** — все 3 marker'а ниже y=260 → `False`.
5. **test_is_tt_feed_empty_xml_returns_false** — пустая строка → `False`.
6. **test_unknown_with_feed_triggers_navigate_and_renav_match** — mock первый verify → unknown, feed-markers present; mock `_navigate_to_profile_tab` returns True; mock второй verify → match. Assert: `_navigate_to_profile_tab` called once, success path breaks, event `tt_post_switch_recovered_via_renav` logged.
7. **test_unknown_with_feed_renav_still_unknown_fails** — оба verify unknown; assert `_fail` called with message containing `tt_post_switch_verify_unrecoverable`.
8. **test_unknown_with_feed_renav_mismatch_falls_through** — re-verify returns mismatch; assert mismatch-path engaged (existing retry logic).
9. **test_unknown_non_feed_fails_immediately** — XML без feed-markers; assert `_navigate_to_profile_tab` NOT called, `_fail` called with `tt_post_switch_verify_unrecoverable` (was_feed=False).
10. **test_publisher_kernel_step_to_category_mapping** — assert `_SWITCHER_STEP_TO_CATEGORY['tt_4_target_profile']` resolves to `tt_post_switch_verify_unrecoverable` (or the equivalent recovery-step entry).
