...
def _try_dismiss_and_redump(self, xml, *, probe_site, target, label, attempt):
    """Return new XML after dismiss, or None if no whitelist match."""
    matched = _tt_try_dismiss_post_switch_modal(xml)
    if matched is None:
        return None
    title_substr, button_text, (bx, by) = matched
    self.p.log_event('info', 'tt_post_switch_modal_dismiss_attempted',
                     meta={'title_substr': title_substr, 'button_text': button_text,
                           'tap_xy': [bx, by], 'probe_site': probe_site,
...
`_tt_try_dismiss_post_switch_modal(xml) -> Optional[tuple[str, str, tuple[int, int]]]`:
...
3. Для каждой `(title_substr, button_text)` пары в whitelist:
   ...
   b. Найти `clickable` `el2` с `el2.label.strip() == button_text`.
   c. Если оба найдены → вычислить center из `el2.bounds = [x1,y1,x2,y2]`:
      `cx = (x1 + x2) // 2`, `cy = (y1 + y2) // 2`. Return `(title_substr, button_text, (cx, cy))`.
4. Return None.
...
|---|---|---|---|
| `tt_post_switch_modal_dismiss_attempted` | `info` | `title_substr, button_text, tap_xy, probe_site, target, attempt` | перед tap dismiss |
...
Кейсы:
1. `modal_phone_email_6514.xml` → returns `('Привязать номер телефона или эл. почту', 'Не сейчас', (cx, 1433))`.
2. `modal_save_login_7307_renav.xml` → returns `('Сохранить данные для входа', 'Не сейчас', (cx, ≈1388))`.
...
Mock `self.p` (publisher proxy — `log_event`, `dump_ui`, `tap`, `adb`) и метод
...
| `tap` промахивается по кнопке | Center-tap по bounds — стандартный паттерн; промах → re-verify=unknown → existing fail. |
