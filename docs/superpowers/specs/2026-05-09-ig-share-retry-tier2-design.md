...
**Dashboard split** (на стороне triage; **Codex round 1 fix P2.2: event name живёт в `meta.category`, не `msg`**):

В терминах event'ов из `publish_tasks.events::jsonb`:
- **Tier 2 success** (recovery hit) — для подсчёта эффективности:
  ```sql
  events[].type='info' AND events[].meta->>'category'='ig_share_long_press_progressed'
  ```
- **Tier 2 exhausted** (escalation не помог): error_code='ig_share_tap_no_progress' + META Tier-2 flag:
  ```sql
  ...
  ```
- **Tier 1-only fail** (pre-deploy хвост или Tier 2 helper exception): same error_code, без tier2_attempted=true в любом event'е:
  ```sql
  pt.error_code='ig_share_tap_no_progress'
