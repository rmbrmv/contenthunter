...
- Два feature flag'а для постепенной активации (см. таблицу в Архитектура).
...
При **всех** flag=false поведение идентично prod (PR #28 + PR #29) — **за исключением RC-B.0** (SAASceneWrapperActivity добавлен в SEED). Это always-on изменение влияет на `_tt_infer_post_publish_success` независимо от flag'ов. Codex v7 round 2, P2#2: явно фиксируем что это не «pure no-op rollout», но direction of change — anti-FP (detector становится строже, не permissive'нее → не может ввести новых ошибочных confirm'ов).
...
1. Deploy кода с **двумя** flag'ами `false` (defaults). Smoke green (pytest pass). RC-B.0 SEED expansion активна independently (always-on).
