...
- False-positive rate `tt_upload_confirmed_post_music_rights` (= confirmed но реально не опубликовано) **target ≤ 5%** при наличии ≥10 events за 24h. (Codex round 3, P2#4: абсолютный «0 FP» нереалистичен для hypothesis-driven detector без видеоверификации — silent-cancel без toast'а может транзитно показать MainActivity. Считаем FP через downstream: задачи с `tt_upload_confirmed_post_music_rights` event'ом которые потом получили `publish_failed_*` ИЛИ у которых contento не появился в feed'е по post-publish-check'у.)
...
Один файл — `publisher_tiktok.py`. Изменения за тремя feature-flag'ами (Codex round 3, P3#1: ниже три, не два):
...
if (os.environ.get('TT_POST_MUSIC_RIGHTS_DETECTION_ENABLED', 'false').lower() == 'true'
    ...
    if iters_since >= 3:  # минимум 9 сек на transition (Codex round 2, P1#2)
        ...
        # Codex round 4, P2#3: 'Загружается'/'Uploading' уже входят в UPLOAD_OK
        # и считаются success'ом — НЕ дублируем как anti-signal, иначе создадим
        # семантический конфликт. Ловим ТОЛЬКО pre-success processing state'ы.
...
| `test_post_music_rights_upload_in_progress_blocks_success` | mock: `MainActivity` + 'Обрабатывается' в ui (НЕ 'Загружается' — оно теперь pure UPLOAD_OK marker, Codex round 4, P2#3) | `is_good_iter=False` (queued state guard) |
| `test_post_music_rights_zagruzhaetsya_treated_as_success_via_upload_ok` | mock: post-MR пути + 'Загружается' в ui | post-MR detector не triggers (UPLOAD_OK ниже catches → upload_confirmed=True через стандартный путь) |
| `test_evidence_only_fires_when_button_exists_but_strict_and_fallback_fail` | XML с substring + generic checkbox + button EXACT, НО checkbox-label НЕ EXACT и title НЕ EXACT | strict=False, fallback=False, evidence-only=True → dump (Codex round 4, P2#2: ловим кейс «button OK, но checkbox/title менялись») |
...
4. Включить post-music-rights success-check:
   ```bash
   sed -i '/^TT_POST_MUSIC_RIGHTS_DETECTION_ENABLED=/d' .env 2>/dev/null || true
