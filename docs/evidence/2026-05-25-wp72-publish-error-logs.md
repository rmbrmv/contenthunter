...
5. **Источник контента = `public.unic_tasks`** (уникализация; `project_id` = validator-id, 84=школа/82=Эль).
   `publish_queue.unic_result_id/unic_task_id` тянут отсюда, НЕ из `validator_unic_content`
   (у 82 и 84 там 0 строк — Эль публикуется при 0, значит `validator_unic_content` к делу не относится;
   привязка к WP #95 ниже была ОШИБОЧНОЙ, см. коррекцию 25.05).
6. **Производство уникализации для школы (84) сломалось 07.05** — `public.unic_tasks` project_id=84:
   до 06.05 включительно статусы `done` (контент шёл в очередь → публикации были), **с 07.05 — `error`**
   (07/09/11.05), затем всплеск ошибок (12.05 cancelled+4 error, 14–15.05, 18.05×6, 19.05×2). Эль (82) —
   `done` (последняя задача slot 27.05). То есть контент для школы перестал генерироваться ровно
   на следующий день после последней публикации.

> **КОРРЕКЦИЯ 25.05 (контролёр):** исходный вывод субагента валил остановку на `validator_unic_content=0`
> и WP #95. Это ОПРОВЕРГНУТО: у активной Эль `validator_unic_content` тоже 0, а она публикуется. Реальная
> причина найдена ниже и проверена по тексту ошибки.

**Корневая причина (проверено по `unic_tasks.error_message`):** задачи уникализации Anecole падают в FFmpeg на
схемах 4 и 5 с ошибкой `Invalid PNG signature 0x3C73766720776964`. Hex `3C 73 76 67 20 77 69 64` = ASCII
**`<svg wid`** → в схему как PNG-оверлей подложен **SVG-файл**; FFmpeg не может его декодировать (`Error while
decoding stream`), задача уникализации завершается `error`, контент не создаётся → очередь не наполняется →
выкладки нет с 06.05.

**Вывод (одной фразой):** школа не публикуется с 06.05, потому что с 07.05 сломана **генерация (уникализация)
контента** — в схемах 4/5 Anecole лежит SVG вместо PNG, FFmpeg падает, новых роликов нет. Это
**контентно-операционная проблема (перезалить корректный PNG-ассет в схемы)**, НЕ баг публикатора и НЕ
`validator_unic_content`/#95. Дополнительно стоит рассмотреть харднинг уникализации (валидация типа ассета /
понятная ошибка вместо падения всего пайплайна).
...
**Остановка школы 06.05** → отдельная корзина: **контентно-операционная** — сломана генерация (уникализация),
схемы 4/5 Anecole содержат SVG вместо PNG, FFmpeg падает с 07.05 (см. §5, КОРРЕКЦИЯ 25.05). НЕ код-баг
публикатора, НЕ WP #95. Кандидат на отдельный WP (главный результат по школе).

**Корзина «код-баг-новый» (по логам публикатора):** ПУСТА. Все publisher-сигнатуры в данных либо уже починены
релизами 18–22.05, либо имеют открытый WP (#117, #131), либо относятся к аккаунтам/устройствам, либо к
стейл-логам апрельской эры. Новых дочерних WP по *публикатору* не требуется.

**НО:** остановка школы (сломанная уникализация Anecole, схемы 4/5, SVG-вместо-PNG) — реальная незакрытая
проблема и кандидат на дочерний WP (контент/операционная, не публикатор). Решение — на чекпойнте с Данилом.
